...
const Gesture = () => {
    ...
    const setupMediaPipe = async () => {
      try {
        ...
        const landmarker = await FaceLandmarker.createFromOptions(vision, {
          ...
          outputFaceBlendshapes: true, // WAJIB untuk mendapatkan blendshapes sebagai proxy embedding
          runningMode: "VIDEO",
          numFaces: 1,
        });
        console.log("landmarker", landmarker);
        faceLandmarkerRef.current = landmarker; // Uncomment ini agar landmarker bisa digunakan
  ...
  const predictWebcam = () => {
    ...
    if (video.currentTime !== lastVideoTimeRef.current) {
      ...
      if (results.faceBlendshapes && results.faceBlendshapes.length > 0) {
        // Gunakan blendshapes sebagai proxy embedding (vector dari scores)
        const liveEmbedding = results.faceBlendshapes[0].categories.map(
          (c) => c.score
        );
        recognizeFace(liveEmbedding); // Panggil fungsi pengenalan
  ...
  const recognizeFace = (liveEmbedding) => {
    if (enrolledFaces.length === 0) {
  ...
  const handleEnroll = () => {
    ...
    if (results.faceBlendshapes && results.faceBlendshapes.length > 0) {
      const newEmbedding = results.faceBlendshapes[0].categories.map(
        (c) => c.score
      );
      setEnrolledFaces([
        ...enrolledFaces,
        { name: enrollName, embedding: newEmbedding },
      ]);
