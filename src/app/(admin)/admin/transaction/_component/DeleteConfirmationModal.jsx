...
export default function DeleteConfirmationModal({
  ...
  const handleDelete = async () => {
    try {
      ...
      alert("Gagal menghapus API Key. Silakan coba lagi.");
    }
  };

  const getDeleteMessage = () => {
    if (isMultiple) {
      return `Apakah Anda yakin ingin menghapus ${itemToDelete.length} API Key yang dipilih?`;
    }
    return `Apakah Anda yakin ingin menghapus API Key "${itemToDelete?.name}"?`;
  };
      ...
      <div className="text-center">
        ...
        <div className="flex items-center justify-center w-full gap-3 mt-7">
          ...
          <Button
            ...
            disabled={deleteUser.isPending}
            className="text-white bg-error-500 hover:bg-error-600"
          >
